...
x = D[:,1:3]
y = D[:,3] # class column of core vs non-core  as 0 and 1
x_tr, x_ts, y_tr, y_ts = train_test_split(x,y,test_size=0.20,random_state=0, stratify=y) 

# Fit the model
reglogModel = LogisticRegression(random_state = 0)
#reglogModel.fit(X_train, y_train)
reglogModel.fit(x_tr, y_tr)

# Predict the probabilities
y_pred = reglogModel.predict_proba(x_ts)
...
precision, recall, thresholds = precision_recall_curve(y_ts, y_pred)
...
for index, elem in enumerate(thresholds):
    ...
    fscore[index] = f1_score(y_ts, y_pred_prob)
